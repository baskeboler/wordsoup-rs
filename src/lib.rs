...
mod tests {
    ...
    fn test_generate() {
        ...
        let mut ws: Wordsoup = match Wordsoup::generate_random_wordsoup(16, 16, 20, &mut d) {
            Err(msg) => {
                println!("Error: {}", msg);
                panic!("Error");
            }
...
impl Wordsoup {
    ...
    pub fn try_add_word(&mut self, word: &str) -> Result<Word, &str> {
        ...
        let r_h = rand::distributions::Range::new(1, self.h);
        let r_w = rand::distributions::Range::new(1, self.w);
        ...
        for i in 0..POS_RETRIES {
            let p = Position {
                x: r_w.ind_sample(&mut r),
                y: r_h.ind_sample(&mut r),
            };
    ...
    pub fn render(&self) -> Result<HashMap<i32, u8>, &str> {
        ...
        let i = self.words.iter();
        i.for_each(|w: &Word| {
            w.cells().iter().for_each(|c: &Cell| {

                res.insert(c.pos.y * self.w + c.pos.x, c.letter);
            });
        });
        for index in 0..self.w * self.h {
            res.entry(index).or_insert_with(|| self.random_letter());
